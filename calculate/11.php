...
$data = mysqli_query($conn,"select * from audusd where ".month(11));

$model_from = mysqli_query($conn, "select * from online onl join audusd au on onl.id=au.id WHERE squared=(select min(squared) from online) and ".month(10));
...
do
{
    ...
    $mape = mape($sum_percentage_error, $n_data);
    	if($eppoach==500 or $mad<0.005)        break;        }while($mse>0.00001);
